...
export class CartService {
  ...
  const products = JSON.parse(localStorage.getItem('cart')).filter((el : IcartProducts )=>{
  ...
  updateQuanity(product:IProducts){
     const products=JSON.parse(localStorage.getItem('cart'))
       ...
       const updateCartProducts=products?.map((cartProduct:IcartProducts)=>{
              ...
              return product._id == cartProduct._id ? { ...cartProduct, quantity: cartProduct.quantity + 1 } : { ...cartProduct}
       })

       localStorage.setItem('cart', JSON.stringify(updateCartProducts));
      //  this.products =updateCartProducts
  }
