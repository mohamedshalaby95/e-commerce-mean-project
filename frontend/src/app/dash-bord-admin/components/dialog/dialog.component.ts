...
export class DialogComponent implements OnInit {
  ...
  constructor(private fb:FormBuilder,private dialog: MatDialog ,   public dialogRef: MatDialogRef<DialogComponent >
   ,private adminService:AdminServicesService ,@Inject(MAT_DIALOG_DATA) public updateProduct:Iproduct) {
    ...
    this.productForm=this.fb.group({
      ...
      image:['',[Validators.required,Validators.pattern(/([a-z\-_0-9\/\:\.]*\.(jpg|jpeg|png|gif))/i)]],
      price:['',[Validators.required]],
      countInStock:['',[Validators.required]],

      // /\[A-Z](jpe?g|png|gif|bmp)$/i
  ...
  ngOnInit(): void {
    ...
    if(this.updateProduct){
     ...
     this.productForm.get('countInStock').setValue(this.updateProduct.countInStock)
...
if(this.actionBtn==='Save'){
  ...
  this.adminService.addProduct(this.productForm.value).subscribe(()=>{
   ...
   this.adminService.updateProduct(this.productForm.value,this.updateProduct._id).subscribe(()=>{
    this.productForm.reset();
