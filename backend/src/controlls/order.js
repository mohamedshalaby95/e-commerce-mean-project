...
async function addOrder(req,res){
  ...
  let order=  new Order({user:req.user,... _.pick(req.body, ["orderitems", "isDeliverd", "isPayment", "status","shippingPrice","totalPrice"])})
...
async function getOrdersUser(req,res){
 ...
 async function getOrder(req,res){
  ...
  if(mongoose.isValidObjectId(req.params.id)){
      ...
      const order= await Order.findById({_id:req.params.id}).populate('user','email shippingAddress')
      if(order){
          res.json(order)
  ...
  async function deleteOrder(req,res){
    ...
    async function getPendingOrdersAdmin(req,res){
        const orders= await Order.find({status:'pending'}).populate('user','email shippingAddress');
        res.json(orders)
     }
...
module.exports={addOrder,getOrdersAdmin,getOrdersUser,getOrder,deleteOrder,getPendingOrdersAdmin,updateStatusOrder,updatePaymentOrder}